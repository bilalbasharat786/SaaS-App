...
// Register New Company (Tenant) & First Admin
router.post('/register-tenant', async (req, res) => {
    try {
        const { companyName, userName, email, password } = req.body;

        const tenantExists = await Tenant.findOne({ name: companyName });
        if (tenantExists) return res.status(400).json({ message: "Company name already taken" });
        ...
        // 2. Hash password & Create Admin User for this Tenant
